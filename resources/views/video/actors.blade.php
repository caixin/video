...
@inject('VideoActors', 'Models\Video\VideoActors')
        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th>{!! sort_title('name', '女优', $route, $order, $search) !!}</th>
            ...
            @foreach ($result as $row)
                <tr>
                    <td>{{ $row['id'] }}</td>
                    <td><a href="{{ route("video.index",['actors'=>$row['name']]) }}">{{ $row['name'] }}</a></td>
                    <td>
                    @if (session('roleid') == 1 || in_array("$controller.save", $allow_url))
                        <button type="button" id="hot1_{{ $row['id'] }}" class="btn {{ $row['hot'] == 1 ? 'btn-info' : 'btn-default' }}" onclick="hot_row({{ $row['id'] }},1)">{{ $VideoActors::HOT[1] }}</button>
                        <button type="button" id="hot0_{{ $row['id'] }}" class="btn {{ $row['hot'] == 0 ? '' : 'btn-default' }}" onclick="hot_row({{ $row['id'] }},0)">{{ $VideoActors::HOT[0] }}</button>
                    @else
                        {{ $VideoActors::HOT[$row['hot']] }}
                    @endif
                    </td>
                </tr>
            @endforeach
